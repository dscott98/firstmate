...
- `no-mistakes-prod-only` is a conditional policy rather than one flat mode: product behavior, security, complex changes, Firstmate safety-critical shared code, and any product-facing, mixed, or uncertain work require full `no-mistakes` validation, taking precedence over every lighter-path category.
  Only otherwise do documentation-only changes, genuinely internal-only tooling, automation, contributor or operator process, and release or submission work ship `direct-PR`.
...
A `no-mistakes` or `no-mistakes-prod-only` project must have an `origin` remote and must complete the initialization procedure below, because the conditional policy retains a full-validation path even when some tasks qualify for `direct-PR`.
