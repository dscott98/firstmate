...
The turn-end extension also provides `/fm-openrouter-sol`, a fixed primary-lock-gated command that uses Pi's current-session model API without changing worker launches or new-session defaults.
