...
state/               runtime records and signals; gitignored
  ...
  .lock-session      trusted Claude session id recorded beside the session lock .lock, whose first line is the owning session's anchor pid; written only by bin/fm-lock.sh under its claim lock, absent when the owner proved no trusted id, and read by bin/fm-session-lock-lib.sh's same-session ownership signal
